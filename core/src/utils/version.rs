...
pub fn bunkernet_version() -> String {
    bunkernet_utils::git::version()
}
...
pub async fn check_version() -> Result<()> {
    ...
    let url = "https://api.github.com/repos/aspectron/bunker-ng/releases/latest";
    let response = Request::new(url)
        .with_user_agent(format!("bunker-ng {current_version} software update check"))
