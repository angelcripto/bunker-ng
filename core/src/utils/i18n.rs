    ...
    if path.ends_with("debug") || path.ends_with("release") {
        ...
        if path.exists() {
            ...
            if local.exists() {
                ...
                // same as bunker-ng settings storage folder: `~/.bunker-ng/`
                let default_storage_folder =
                    bunkernet_wallet_core::storage::local::default_storage_folder();
    ...
    if path.ends_with("debug") || path.ends_with("release") {
        ...
        if in_same_folder.exists() {
            ...
            if local.exists() {
                ...
                Ok(storage_folder.join("bunker-ng.i18n.json"))
            }
        }
    }
}
