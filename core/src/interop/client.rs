...
pub use bunkernet_wallet_core::api::transport::BorshCodec;
pub use bunkernet_wallet_core::api::transport::{EventHandler, WalletServer};
...
impl Client {
    ...
    pub async fn handle_message(&self, target: Target, data: Vec<u8>) -> Result<Option<Vec<u8>>> {
        match target {
            Target::Wallet => {
                let event = Box::new(bunkernet_wallet_core::events::Events::try_from_slice(&data)?);

                self.application_events
                    .sender
                    .try_send(bunker_ng_core::events::Events::Wallet { event })
                    .unwrap();
