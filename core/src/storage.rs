...
impl Storage {
    ...
    pub fn storage_root(&self) -> PathBuf {
        self.storage_root
            ...
            .unwrap()
            .clone()
            .or_else(|| Some(bunkerd_lib::daemon::get_app_dir()))
            .unwrap()
    }
    ...
    pub fn update(&self, options: Option<StorageUpdateOptions>) {
        ...
        let bunkernet_app_dir = self.storage_root();
        if !bunkernet_app_dir.exists() {
            return;
        }
        ...
        spawn(async move {
            ...
            let paths = std::fs::read_dir(bunkernet_app_dir).unwrap();
            for path in paths {
                let path = path?.path();
    ...
    fn update_folder_size(&self, network: Network, folder_size: u64, path: PathBuf) {
        use bunkernet_metrics_core::data::as_data_size;
