...
impl<'core> Menu<'core> {
    ...
    pub fn render_single_pane_menu_open(&mut self, ui: &mut Ui, device: &Device) {
        ...
        // ui.image(source)
        // ui.add(
        //     Image::new(ImageSource::Bytes { uri : Cow::Borrowed("bytes://logo-transparent.svg"), bytes : Bytes::Static(crate::app::BUNKER_NG_ICON_TRANSPARENT_SVG)})
    ...
    pub fn render_single_pane_menu(&mut self, ui: &mut Ui, device: &Device) {
        ui.add(
            Image::new(ImageSource::Bytes {
                ...
                bytes: Bytes::Static(crate::app::BUNKER_NG_ICON_TRANSPARENT_SVG),
            })
            .fit_to_exact_size(Vec2::splat(device.top_icon_size()))
            .maintain_aspect_ratio(true)
            // .texture_options(TextureOptions::NEAREST),
                ...
                cols[0].horizontal(|ui| {
                    if device.single_pane() {
                        ...
                        if self.core.device().single_pane() {
                            ui.menu_button("BunkerNet NG", |ui| {
